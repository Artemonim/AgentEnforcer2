...
**Уточнение**:
- Считать только строки исполняемого кода; игнорировать пустые строки, комментарии и чистую документацию.
  - Пример: в Rust юнит-тесты могут находиться в том же файле (например, `#[cfg(test)] mod tests`), поэтому `line-limits` должен опираться на строки исполняемого кода, а не на простой счётчик строк файла.
